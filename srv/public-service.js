...
const debug = require('debug')('srv:public-service');
...
module.exports = cds.service.impl(async function () {
    ...
    this.on('prefillCircularityMetrics', Assessments, async (req) => {
        try {
            ...
            const circularityData = await SELECT.from(RecyclingCountries)
                ...
                console.log(circularityData.length);
    ...
    this.on('testAlertNotification', async (req) => {
        // send generic alert using Alert Notification
        alertNotification({
            type: 'GENERIC',
            data: {
                body: JSON.stringify('Generic Alert'),
