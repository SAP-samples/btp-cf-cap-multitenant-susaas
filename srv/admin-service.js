...
module.exports = cds.service.impl(async function () {
    ...
    if (cds.env.profiles.includes('production')) {
        this.before("SAVE", Users, async (req) => {
            ...
            let tenantApprouterUrl = modifiedEndpoint(process.env.approuterUrl,`${tenantSubdomain}-susaas-${vcapApplication.space_name}`)
            ...
            try{
                if (req.event !== 'UPDATE') {
                    let iasLocation = await userManagement.createIASUser({
    ...
    if (cds.env.profiles.includes('production')) {
        this.before("DELETE", 'Users', async (req) => {
            ...
            await userManagement.deleteIASUser(user.iasLocation);
        })
    }
});