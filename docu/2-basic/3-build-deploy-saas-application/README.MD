...
Before the deployment of the sample application, you need to build the project and do some basic steps which are required for security purposes. After deployment make sure required credential values are stored in the credential store created during deployment.  
...
1.3. From the root directory please excecute the following npm script to create catalog IDs and credentials for your service broker instance. 
...
> **Important** - Please make sure to store the hashed and plaintext password in a secure place especially in a productive scenario which you and also other SAP BTP platform administrators can access at any time!  
...
1.6. Decide if you want to deploy the sample data csv files with your project. We recommend to use the SaaS API to push the respective data after deployment of the solution.
...
1.9. Once the deployment has finished, you're almost ready to go. To support some automation steps like the creation of routes or deployment of the API Service Broker in the consumer subaccounts, make sure not to miss the next step before settings up your first consumer subaccount. 


## 2. Setup the Credential Store

Before you learn how to subscribe new tenants in the next part of the mission, you need to provide two credentials in the Credential Store. These credentials are essential for some parts the automated subscription process. 
...
2.2. In the instance management, please switch to the **Credential Store** menu click on **Create Namespace**. 
...
Provide the e-mail address (Username) and password (Value) of a SAP BTP user which is used for automation purposes. Make sure this user has the **Subaccount Administrator** role-collection in your **Provider subaccount** and the **Space Developer** role in the respective Cloud Foundry space. In a Trial context your current user should be sufficient. 

> **Hint** - If you don't want to use a personal/named user for this purpose, we recommend to use a custom IdP and to define a technical user there. The usage of **P or S-User** for technical tasks is possible but especially for productive scenarios not recommended by SAP. 
...
As Username please use the value **broker-user**. 
